...
> {
  ...
  play = () => {
    if (!this.#renderer)
      throw new Error("Frames could not start: : No WebGLRenderer attached");
    this.#paused = false;
    this.#clock.start();
  ...
  pause = () => {
    if (!this.#renderer)
      throw new Error("Frames could not pause: : No WebGLRenderer attached");
    this.#paused = true;
    this.#clock.stop();
  ...
  interpolate = ({
    ...
    return new Promise<number>((res) => {
      ...
      if (typeof onStart === "function")
        onStart({ value, time: 0, deltaTime: 0.08 });
      this.add(tick);
    });
  };
}
